...
import java.util.Set;
	...
	public class ModelRocchioOrigin {
...
//		HashMap<Integer, Integer> shared_featurePos = new HashMap<Integer,Integer>();		
		...
		ModelRocchioOrigin(RawData dataset) {
		...
		public void init(String goldenSetString)  {
//			dataset = rawData;
			weight_gain = 1.1;
			weight_discount = 0.9;
			...
			if(goldenSetString.contains("capitals"))
				thres_candidate_pool = 0.015;
			else if(goldenSetString.contains("it_companies"))
				thres_candidate_pool = 0.015;
			else
...
//			loadGoldSet();   /*move that to data class*/
		...
		private void constructCandidatePool() {
			...
			ArrayList<Ne_candidate> list2 = new ArrayList<Ne_candidate>();
			train();
			HashSet<Integer> wholePool = new HashSet<Integer>();
			for(Integer nameInteger : dataset.ne_feature_logpmi.keySet()){
				wholePool.add(nameInteger);
			}
			
		//	wholePool = (HashSet<Integer>) dataset.ne_feature_logpmi.keySet();
			list1 = apply_classifier(wholePool, 1);
			list2 = apply_classifier(wholePool, 2);
			
			Collections.sort(list1,Collections.reverseOrder());
			Collections.sort(list2,Collections.reverseOrder());
			
			getCandidatePool(list1, list2);
			
		}
		
		private void getCandidatePool(ArrayList<Ne_candidate> list1, ArrayList<Ne_candidate> list2){
		
			ArrayList<Ne_candidate> words_sim = new ArrayList<Ne_candidate>();

			// to speed up, use 20000 instead of words_sim.size()
			int max_index = 20000;
			if(list1.size() < max_index)
				max_index = list1.size();
			if(list2.size() < max_index)
				max_index = list2.size();

			for(int cc = 0; cc < max_index; cc++) {
				Ne_candidate f = new Ne_candidate();

				double sim1 = 0, sim2 = 0;
				sim1 = list1.get(cc).sim;

				for(int cc2 = 0; cc2 < max_index; cc2++) {
					if(list2.get(cc2).iNE == f.iNE) {
						sim2 = list2.get(cc2).sim;
						break;
					}
				}

				f.iNE = list1.get(cc).iNE;
